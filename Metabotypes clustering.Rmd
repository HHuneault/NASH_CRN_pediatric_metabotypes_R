...
hispanic_count <- sum(metabotypes$HISPANIC == 1, na.rm = TRUE) 
...
# Define custom test function compatible with gtsummary
custom_test <- function(data, variable, by, ...) {
  ...
  if (any(tbl < 5)) {
    test <- tryCatch(
      fisher.test(tbl, workspace = 2e5, simulate.p.value = TRUE),  
      error = function(e) {
        list(p.value = NA)  
      }
    )
...
p <- ggplot(metabotypes_original, aes(x = cluster, y = WC..cm., fill = cluster)) +
  ...
  stat_compare_means(comparisons = list(c("Adipo-lipid-SBP", "Inflammatory-fibrotic"), 
                                        ...
                                        c("Inflammatory-fibrotic", "Early-mild")),  
                     method = "wilcox.test",  
                         label = "p.signif",
...
calculate_p_value <- function(cluster1, cluster2) {
  data_subset <- merged_data %>%
    ...
    dplyr::select(cluster, Adv_fibrosis) 
...
bar_chart <- bar_chart + 
  ...
  geom_segment(aes(x = 3, xend = 1, y = 110, yend = 110), size = 0.5) +  
  annotate("text", x = 2, y = 112, label = p_values$stars[1], size = 6) +  
  
  # Line and stars for Inflammatory-fibrotic vs. Adipo-lipid-SBP
  geom_segment(aes(x = 1, xend = 2, y = 105, yend = 105), size = 0.5) +  
  annotate("text", x = 1.5, y = 107, label = p_values$stars[2], size = 6) + 
  
  # Line and stars for Early-mild vs. Adipo-lipid-SBP
  geom_segment(aes(x = 2, xend = 3, y = 100, yend = 100), size = 0.5) +  
  annotate("text", x = 2.5, y = 102, label = p_values$stars[3], size = 6)  
